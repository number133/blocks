...
function createUI() {
    ...
    game.create.grid('uiGrid', 32 * 16, 32, 32, 32, '#ff5500');
    ...
    paletteArrow = game.add.sprite(40, 36, 'arrow');
...
function createEventListeners() {
    ...
    game.input.onDown.add(onSelected, this);
...
function fillArea() {
	for(var i=0; i<spriteWidth; i++){
		for(var j=0; j<spriteHeight; j++){
			...
			nextColor();
			nextColor();
		    data[j][i] = pmap[colorIndex];
	        canvas.rect(i * canvasZoom, j * canvasZoom, canvasZoom, canvasZoom, color);
	        preview.rect(i * previewSize, j * previewSize, previewSize, previewSize, color);	    
		}
	}
}

function onSelected(pointer) {
    ...
    if(isSelected){
    	var colorSymbolFirst = data[selectedY][selectedX];
    	var colorSymbolSecond = data[y][x];

    	data[y][x] = colorSymbolFirst;
		var c = game.create.palettes[palette][colorSymbolFirst];
        canvas.rect(x * canvasZoom, y * canvasZoom, canvasZoom, canvasZoom, c);
        preview.rect(x * previewSize, y * previewSize, previewSize, previewSize, c);

    	data[selectedY][selectedX] = colorSymbolSecond;
    	c = game.create.palettes[palette][colorSymbolSecond];
        canvas.rect(selectedX * canvasZoom, selectedY * canvasZoom, canvasZoom, canvasZoom, c);
        preview.rect(selectedX * previewSize, selectedY * previewSize, previewSize, previewSize, c);
    	isSelected = false;
    } else {
    	selectedX = x;
    	selectedY = y;
